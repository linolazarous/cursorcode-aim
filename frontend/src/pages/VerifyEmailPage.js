...
export default function VerifyEmailPage() {
  ...
  const verifyEmail = useCallback(
    async (token) => {
      try {
        // Remove extra /api because axios instance already adds /api
        ...
        <div className="bg-void-paper border border-white/5 rounded-xl p-8 text-center">
          ...
          {status === "success" && (
            <>
              ...
              <h2 className="font-outfit font-bold text-2xl text-white mb-2">Email Verified!</h2>
          ...
          {status === "error" && (
            <>
              ...
              <h2 className="font-outfit font-bold text-2xl text-white mb-2">Verification Failed</h2>
              <p className="text-zinc-400 mb-6">{message}</p>
              <Button
